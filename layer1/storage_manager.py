...
class StorageManager:
    ...
    def _project_scoped_table_name(self, prefix: str, original_name: str) -> str:
        """Build a project-scoped storage table name."""
        sanitized_project = self._project_token()
        sanitized_name = self._sanitize_name(original_name)

        prefix_token = prefix.strip().lower()
        prefix_with_sep = f"{prefix_token}_"
        if sanitized_name.startswith(prefix_with_sep):
            sanitized_name = sanitized_name[len(prefix_with_sep):]

        base_identifier = "_".join(part for part in (prefix_token, sanitized_project) if part)
        candidate = (
            f"{base_identifier}_{sanitized_name}" if sanitized_name else base_identifier
        )

        max_length = 63
        if len(candidate) <= max_length:
            return SQLQueryValidator.validate_table_name(candidate)

        import hashlib

        hash_suffix = hashlib.md5(candidate.encode("utf-8")).hexdigest()[:8]
        if sanitized_name:
            available = max_length - len(base_identifier) - len(hash_suffix) - 2
            if available > 0:
                truncated = sanitized_name[:available].rstrip("_")
                if truncated:
                    candidate = f"{base_identifier}_{truncated}_{hash_suffix}"
                    return SQLQueryValidator.validate_table_name(candidate)
            candidate = f"{base_identifier}_{hash_suffix}"
            return SQLQueryValidator.validate_table_name(candidate)

        trimmed_base = base_identifier[: max_length - len(hash_suffix) - 1].rstrip("_")
        fallback = (
            f"{trimmed_base}_{hash_suffix}" if trimmed_base else f"{prefix_token}_{hash_suffix}"
        )
        return SQLQueryValidator.validate_table_name(fallback)
